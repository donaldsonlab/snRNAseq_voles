...
```{r}
all_cells <- read.csv("ani_mod_scores_allcells.csv")
...
```{r}
mod_genes <- read.csv("new_clusts_hotspot-gene-modules.csv")
...
setwd("output/")
for (mod in "Module.6") {
  corr_plot(merged_data, color = "deeppink4", font_size = 20, save = TRUE)
}

for (mod in "Module.11") {
  corr_plot(merged_data, color = "chartreuse4", font_size = 20, save = TRUE)
}
```
